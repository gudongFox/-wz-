/*
creat by jiangxiaojin 20200408
for upload file and retrun value
 modle="add  edit remove" 分别对应 上传 编辑 删除
*/
/*
 * version: 1.7.3
 * number:002
...
var ntkoBrowser = {
    ...
    openWindow:function( strURL,IsShowOnExtendMonitor,ProductCaption, ntkoBrowserProductKey, ntkoBrowserNoExpireKey, ntkoBrowserOptions )
    {
        ...
        jsonValue += '中国五洲工程设计集团有限公司';
        jsonValue += '"';
        jsonValue += ',"ProductKey":"';
        jsonValue += 'C1CA4863EEDA5E394076D4D0C6AF8637588A3B13';
        jsonValue += '"';
        jsonValue += ',"NoExpireKey":"';
        jsonValue += '9203EBABBA4688404CF13EC31C260C21';
        jsonValue += '"';
...
/*function getFileName(fullPath) {
    var location = fullPath.lastIndexOf('/');
    return fullPath.substring(location + 1);
}

function removeFile(rootId) {
    var input = document.getElementById(rootId);
    var id = input.getAttribute("value");
    $.ajax({
        type:"get",
        url: "/wuzhou/file/remove.json",
        data:{id:id},
        dataType:"json",
        success:function(data){
            if (data.ret){
                ...
                if (rootId.indexOf("contextFile_")>-1){
                    initButtons(input,0);
                }else {
                    initButtons1(input,0);
                }
            }
        }
    })
...
function editRedHeadModel(path) {
    $("#selectRedHeadModal").modal("show");

}

function saveRedHeadModel(path) {

    $("#selectRedHeadModal").modal("hide");
}

function openContext(id){
    var input = document.getElementById(id);
    var fileid= input.getAttribute("tableName");
    var path="/wuzhou/file/downloadRedHead/"+fileid;
    if (id.indexOf('contextFile_')>-1){
         path="/wuzhou/file/downloadContent/"+fileid;
    }
    ...
    if(ntkoed){
        console.log(path);
        if (id.indexOf('contextFile_')>-1){
            ntkoBrowser.openWindow("/nochange/rh-office-control/contextEditindex.html?path=" + path);
        }else {
            ntkoBrowser.openWindow("/nochange/rh-office-control/redEditindex.html?path=" + path);
        }
    }
    else{
...
function  initButtons(inputElment,index) {
    var rootId="contextFile_"+index;
    var id="context_"+index;
    var value= inputElment.getAttribute("tableName");;
    var path="/wuzhou/file/downloadContent/"+value;
    var fileName=inputElment.getAttribute("fileName");
    var stringHtml="";
    stringHtml+=inputElment.outerHTML;
    stringHtml+=("<a href='"+path+"'>"+fileName+"</a>&nbsp;&nbsp;");
    var modle=inputElment.getAttribute("modle");
    if(modle.indexOf("add")>-1){
        stringHtml+=("<span id=\"btnUpload\" class=\"btn btn-sm default fileinput-button\" >" +
            "<i class=\"fa fa-cloud-upload\"></i>" +
            "<span>上传</span>" +
            "<input id=\""+id+"\" type=\"file\" name=\"singleUpload\"  accept=\".doc,.docx\"/>" +
            "</span>");
    }
    if(modle.indexOf("edit")>-1&&fileName!=undefined&&fileName!=""){

        stringHtml+=("<span id='btnEdit' onclick=\"openContext('"+rootId+"')\" class='btn btn-sm default fileinput-button'>" +
            "<i class='fa fa-edit'></i>" +
            "<span>编辑</span>" +
            "</span>");
    }
    if(modle.indexOf("remove")>-1&&fileName!=undefined&&fileName!=""){
        stringHtml+=("<span id=\"btnDelete\"onclick=\"removeFile('"+rootId+"')\" class=\"btn btn-sm default fileinput-button\">" +
            "<i class=\"fa fa-trash\"></i>" +
            "<span>删除</span>" +
            "</span>");
    }

    inputElment.parentElement.innerHTML=stringHtml;
    initUpload(inputElment,index);
}

function  initButtons1(inputElment,index) {
    var rootId="redHeaderFile_"+index;
    var id="readHeader_"+index;
    var value= inputElment.getAttribute("tableName");;
    var path="/wuzhou/file/downloadRedHead/"+value;
    var fileName=inputElment.getAttribute("fileName");
    var input = document.getElementById("contextFile_0");
    var fileNameContent=input.getAttribute("fileName");
    var stringHtml="";
    stringHtml+=inputElment.outerHTML;
    stringHtml+=("<a href='"+path+"'>"+fileName+"</a>&nbsp;&nbsp;");
    var modle=inputElment.getAttribute("modle");
    ...
    if(modle.indexOf("redhead")>-1&&fileNameContent!=undefined&&fileNameContent!=""){
        stringHtml+=("<span id='btnEdit' onclick=\"openContext('"+rootId+"')\" class='btn btn-sm default fileinput-button'>" +
            "<i class='fa fa-edit'></i>" +
            "<span>套红</span>" +
            "</span>");
    ...
    inputElment.parentElement.innerHTML=stringHtml;
    initUpload1(inputElment,index);
}

function initUpload(sender,index) {
    var rootId="contextFile_"+index;
    var id="context_"+index;
    $('#context_0').fileupload({
        ...
        add: function (e, data) {
            var input = document.getElementById(rootId);
            var tableKey=  input.getAttribute("tableKey");
            var tableName= input.getAttribute("tableName");
            data.formData={tableName:tableName,tableKey:tableKey,userLogin:user.userLogin,fileType:1}
            data.submit();
        },
            ...
            if (result.ret) {
                toastr.success("上传成功!");
                var input = document.getElementById(rootId);
                input.setAttribute("value",result.data.id);
                input.setAttribute("fileName",result.data.fileName);
                input.setAttribute("tableKey",result.data.tableKey);
                initButtons(input,index);
            } else {
                toastr.error("上传失败!");
            }
...
function initUpload1(sender,index) {
    var rootId="redHeaderFile_"+index;
    var id="readHeader_"+index;
    $('#'+id).fileupload({
        url: '/wuzhou/file/receive.do',
        ...
        add: function (e, data) {
            ...
            var tableKey=  input.getAttribute("tableKey");
            var tableName= input.getAttribute("tableName");
            data.formData={tableName:tableName,tableKey:tableKey,userLogin:user.userLogin,fileType:0}
            data.submit();
        },
        done: function (e, data) {
            var result = JSON.parse(data.result);
...
function delayLoad(){
    ...
    var inputElments= document.getElementsByClassName("context-file");
    for(var i=0;i<inputElments.length;i++){
        var rootId="contextFile_"+i;

        var inputElment=inputElments[i];
        inputElment.setAttribute("id",rootId);
        initButtons(inputElment,i);
    }
    var inputElments1= document.getElementsByClassName("redHeader-file");
    for(var i=0;i<inputElments1.length;i++){
        var rootId="redHeaderFile_"+i;

        var inputElment=inputElments1[i];
        inputElment.setAttribute("id",rootId);
        initButtons1(inputElment,i);
    }
}
