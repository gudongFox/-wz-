...
import com.cmcu.mcc.finance.service.FiveFinanceCompanyBankService;
...
@RestController
@RequestMapping("/five/finance/companyBank")
public class FiveFinanceCompanyBankController {
    @Autowired
    FiveFinanceCompanyBankService fiveFinanceCompanyBankService;

    @PostMapping("/getModelById.json")
    public JsonData getModelById(int id) throws ParseException {
        return JsonData.success(fiveFinanceCompanyBankService.getModelById(id));
    }

    @PostMapping("/getNewModel.json")
    public JsonData getNewModel(String userLogin,String uiSref) {
        return JsonData.success(fiveFinanceCompanyBankService.getNewModel(userLogin,uiSref));
    }

    @PostMapping("/remove.json")
    public JsonData remove(int id, String userLogin) {
        fiveFinanceCompanyBankService.remove(id, userLogin);
        return JsonData.success();
    }

    @PostMapping("/update.json")
    public JsonData update(@RequestBody FiveFinanceCompanyBankDto item) {
        Assert.state(StringUtils.isNotEmpty(item.getOperateUserLogin()), "操作人不能为空!");
        fiveFinanceCompanyBankService.update(item);
        return JsonData.success();
    }
    ...
    public JsonData listPagedData(String userLogin,String uiSref, int pageNum, int pageSize) {
        ...
        PageInfo pageInfo = fiveFinanceCompanyBankService.listPagedData(params, userLogin, uiSref, pageNum, pageSize);
        return JsonData.success(pageInfo);
    }

    @PostMapping("/selectAll.json")
    public JsonData selectAll(String userLogin,String uiSref){
        return JsonData.success(fiveFinanceCompanyBankService.selectAll(userLogin,uiSref));
    }
